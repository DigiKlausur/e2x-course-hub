...
└── spawn_role_options: {SpawnRole → SpawnRoleOptions}   # SpawnRole: student, grader
    ├── profile_options
    │   ├── default_profile: str
...
`SpawnRole` only has two values — `student` and `grader` — because it's a configuration
axis: the set of environments a course/term admin can pick an image/tier/profile for.
Read-only access to the grader environment (e.g. for observers) is not a third
environment; it's the `grader` environment spawned with `SpawnSelection.course_readonly`
set. See `SpawnSelection` below.
...
| `SpawnSelection` | The selection configured for a course/term: role + image / tier / profile, plus `course_readonly` (whether the course directory is mounted read-only for this spawn — never configured, always derived from the user's permission at spawn time). A field left `None` means "not configured → catalog default" |
