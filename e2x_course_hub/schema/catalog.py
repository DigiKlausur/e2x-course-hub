...
from .selection import ImageSelection, SpawnSelection
...
class InfrastructureCatalogOptions(BaseModel):
    ...
    def assert_image_selection_exists(self, image_selection: ImageSelection) -> None:
        self.image_family_options.assert_family_and_tag_exists(
            image_selection.family, image_selection.tag
        )
    ...
    def get_default_image_selection(self) -> ImageSelection:
        return ImageSelection(
            family=self.image_family_options.default_family,
            tag=self.image_family_options.families[
                self.image_family_options.default_family
            ].default_tag,
        )

    def get_default_spawn_selection(self, spawn_role: SpawnRole) -> SpawnSelection:
        if spawn_role not in self.spawn_role_options:
            raise UnknownSpawnRoleError(spawn_role)
        role_options = self.spawn_role_options[spawn_role]
        return SpawnSelection(
            spawn_role=spawn_role,
            image=self.get_default_image_selection(),
            resource_tier_name=role_options.resource_tier_options.default_tier,
            profile_name=role_options.profile_options.default_profile,
        )
